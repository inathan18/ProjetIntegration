document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch(
      ...
      .then((data) => {
        ...
        suppressionDuplicats(municipalite)
            .forEach(element => { output += `<option>${element}</option>`; })

        suppressionDuplicats(regions).forEach(
            element => { output2 += `<option>${element}</option>`; })
